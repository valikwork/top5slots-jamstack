...
import AvailableGamesAccordion from './dropdown/AvailableGamesAccordion'
import PaymentMethodsAccordion from './dropdown/PaymentMethodsAccordion'
import Parser from 'html-react-parser'
import { useHistory, useParams, useLocation } from "react-router-dom";
import queryString from 'query-string'
import withLocation from "../hoc/withLocation"

function TabsFilterSection({ categoriesIncludingAllCasinos, location, navigate, search }) {
    ...
    const filtrationHandler = (filterSettings) => {
        ...
        //Copy array of objects without referece
        const newCategReviews = JSON.parse(JSON.stringify(categoriesIncludingAllCasinos));
        
        let activeFilters = {}
        for (const [key, valueArray] of Object.entries(filterSettings)) {
            if(Array.isArray(valueArray) && valueArray.length){
                activeFilters[key] = valueArray
            }
        }

        const mainFiltration = (casino) => {
            //Must return true or false
            let res = true;
            
            for (const [key, valueArray] of Object.entries(activeFilters)) {
                ...
                if(key === 'withdrawalLimit'){
                    for (let i = 0; i < valueArray.length; i++) {
                        if(casino.cpt_reviews.withdrawalLimit > valueArray[i]){
                            res = true
                        } else {
                            res = false
                        }
                    ...
                    for (let i = 0; i < valueArray.length; i++) {
                        for (let j = 0; j < casino.cpt_reviews[key].length; j++) {
                            if(casino.cpt_reviews[key][j]['slug'] === valueArray[i]){
                                res = true
                                break;
                            } else {
                                res = false
            ...
            for (const item of newCategReviews) {
                if(item.hasOwnProperty('reviews')){
                    item.reviews.nodes = item.reviews.nodes.filter((casino) => {
                        return mainFiltration(casino)
                    })
                }
            }
            setCategReviewsToRender(newCategReviews)
        } else {
            setCategReviewsToRender(categoriesIncludingAllCasinos)
        }
    ...
    useEffect(() => {
        ...
        //     for (const [key, arrValue] of Object.entries(filterSettings)) {
        ...
        //         }   
        //     }
        //     // navigate(`${location.pathname}${searchParamString}`)
        //     // history.push(`${history.location.pathname}${searchParamString}`)
        // } else {
        //     // history.push(history.location.pathname)
        // }
        
        filtrationHandler(filterSettings)
        console.log(categReviewsToRender);
    }, [filterSettings])

    useEffect(() => {
        // PUT 'SHOW' IN QUERY PARAMS   @TODO(?)

        // const searchParams = queryString.parse(history.location.search)
        // const { show } = searchParams
        // console.log('show', show);
        // if(!show){
        //     history.push(`${history.location.pathname}?${history.location.search}show=${reviewsToShow}`)
        // } else {
        //     searchParams.show = reviewsToShow
                                ...
                                <div className="tab-content__wrapper row" >
                                    <div className="column small-12">
                                        <div className="tab-content__reviews-wrap">
                                            ...
                                            <button label={category.name} onClick={() => setReviewsToShow(reviewsToShow + 6)} className="btn btn-arrow btn-center btn-big" to={category.uri}>Show more casinos</button>
                                        </div>
                                    </div>
                                </div> 
                            </div>
...
export default withLocation(TabsFilterSection)